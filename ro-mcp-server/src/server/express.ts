...
export class ExpressServer {
    ...
    this.app.use((req: Request, _res: Response, next: NextFunction) => {
      ...
      if (!req.path.match(/^\/(health|$)/)) {
        ...
        if (tenantHeaders) {
          ...
          logger.debug(`Set tenant context for request: ${tenantHeaders.tenantId}/${tenantHeaders.dealerId}/${tenantHeaders.userId}`);
        } else {
          logger.warn(`Missing tenant headers for request: ${req.method} ${req.path}`);
        }
      }
      next();
    });
  ...
  private setupRoutes(): void {
    ...
    this.app.get('/ro-service/health', async (_req: Request, res: Response) => {
      try {
  ...
  public start(): void {
    this.app.listen(CONFIG.SERVER.PORT, CONFIG.SERVER.HOST, () => {
      logger.info(`RO MCP Express Server running on http://${CONFIG.SERVER.HOST}:${CONFIG.SERVER.PORT}`);
    });
  }
