...
class PartService {
    constructor() {
        ...
        this.client = axios_1.default.create({
            baseURL: config_1.config.partService.baseUrl,
            timeout: config_1.config.partService.timeout,
            headers: {
    ...
    setTenantContext(tenantHeaders) {
        ...
        logger_1.logger.debug('Tenant context set', {
    ...
    clearTenantContext() {
        this.currentTenantHeaders = null;
        logger_1.logger.debug('Tenant context cleared');
    }
    getTenantContext() {
        if (!this.currentTenantHeaders)
            return null;
    ...
    setCache(key, value, ttl) {
        ...
        logger_1.logger.debug(`Cache set: ${tenantKey}`);
    }
    getCache(key) {
        const tenantKey = this.getCacheKey(key);
        const value = this.cache.get(tenantKey);
        if (value) {
            logger_1.logger.debug(`Cache hit: ${tenantKey}`);
        }
        else {
            logger_1.logger.debug(`Cache miss: ${tenantKey}`);
        }
        return value;
    }
    deleteCache(key) {
        ...
        logger_1.logger.debug(`Cache deleted: ${tenantKey}`);
    }
    // Cache invalidation patterns
    invalidatePartCaches(partId) {
        const patterns = ['parts_all', 'parts_search_', 'parts_category_', 'parts_brand_'];
        if (partId) {
            patterns.push(`part_${partId}`);
        }
        patterns.forEach(pattern => {
            const keys = this.cache.keys().filter((key) => key.includes(this.getCacheKey(pattern)));
            if (keys.length > 0) {
                this.cache.del(keys);
                logger_1.logger.debug(`Invalidated cache keys: ${keys.join(', ')}`);
            }
        });
    }
    getCacheStats() {
    ...
    clearCache() {
        ...
        if (tenantContext) {
            ...
            const tenantKeys = this.cache.keys().filter((key) => key.includes(tenantPrefix));
            if (tenantKeys.length > 0) {
                this.cache.del(tenantKeys);
                logger_1.logger.info(`Cleared ${tenantKeys.length} tenant-specific cache entries`);
            }
        }
        else {
    ...
    async getAllParts() {
        ...
        try {
            logger_1.logger.info('Fetching all parts');
            ...
            logger_1.logger.error('Error fetching all parts:', error);
    ...
    async getPartById(id) {
        ...
        try {
            logger_1.logger.info(`Fetching part by ID: ${id}`);
            ...
            logger_1.logger.error(`Error fetching part ${id}:`, error);
    ...
    async getPartByPartNumber(partNumber) {
        ...
        try {
            logger_1.logger.info(`Fetching part by part number: ${partNumber}`);
            ...
            logger_1.logger.error(`Error fetching part ${partNumber}:`, error);
            throw new Error(`Failed to fetch part with number ${partNumber}`);
        }
    }
    async createPart(part) {
        try {
            logger_1.logger.info('Creating new part:', part);
            const response = await this.client.post('/api/parts', part);
            return response.data;
        }
        catch (error) {
            logger_1.logger.error('Error creating part:', error);
            throw new Error('Failed to create part');
        }
    }
    async updatePart(id, part) {
        try {
            logger_1.logger.info(`Updating part ${id}:`, part);
            const response = await this.client.put(`/api/parts/${id}`, part);
            return response.data;
        }
        catch (error) {
            logger_1.logger.error(`Error updating part ${id}:`, error);
            throw new Error(`Failed to update part with ID ${id}`);
        }
    }
    async deletePart(id) {
        try {
            logger_1.logger.info(`Deleting part ${id}`);
            await this.client.delete(`/api/parts/${id}`);
        }
        catch (error) {
            logger_1.logger.error(`Error deleting part ${id}:`, error);
            throw new Error(`Failed to delete part with ID ${id}`);
        }
    }
    async getPartsByCategory(category) {
        try {
            logger_1.logger.info(`Fetching parts by category: ${category}`);
            const response = await this.client.get(`/api/parts/category/${encodeURIComponent(category)}`);
            return response.data;
        }
        catch (error) {
            logger_1.logger.error(`Error fetching parts by category ${category}:`, error);
            throw new Error(`Failed to fetch parts in category ${category}`);
        }
    }
    async getPartsByBrand(brand) {
        try {
            logger_1.logger.info(`Fetching parts by brand: ${brand}`);
            const response = await this.client.get(`/api/parts/brand/${encodeURIComponent(brand)}`);
            return response.data;
        }
        catch (error) {
            logger_1.logger.error(`Error fetching parts by brand ${brand}:`, error);
            throw new Error(`Failed to fetch parts from brand ${brand}`);
        }
    }
    async getPartsByRepairOrder(repairOrderId) {
        try {
            logger_1.logger.info(`Fetching parts by repair order: ${repairOrderId}`);
            const response = await this.client.get(`/api/parts/repair-order/${repairOrderId}`);
            return response.data;
        }
        catch (error) {
            logger_1.logger.error(`Error fetching parts by repair order ${repairOrderId}:`, error);
            throw new Error(`Failed to fetch parts for repair order ${repairOrderId}`);
        }
    }
    // Inventory Management
    async getLowStockParts(threshold = 10) {
        try {
            logger_1.logger.info(`Fetching low stock parts with threshold: ${threshold}`);
            const response = await this.client.get(`/api/parts/low-stock?threshold=${threshold}`);
            return response.data;
        }
        catch (error) {
            logger_1.logger.error('Error fetching low stock parts:', error);
            throw new Error('Failed to fetch low stock parts');
        }
    }
    async getOutOfStockParts() {
        try {
            logger_1.logger.info('Fetching out of stock parts');
            const response = await this.client.get('/api/parts/out-of-stock');
            return response.data;
        }
        catch (error) {
            logger_1.logger.error('Error fetching out of stock parts:', error);
            throw new Error('Failed to fetch out of stock parts');
        }
    }
    async updateStock(id, quantity) {
        try {
            logger_1.logger.info(`Updating stock for part ${id} to ${quantity}`);
            const response = await this.client.put(`/api/parts/${id}/stock`, { quantity });
            return response.data;
        }
        catch (error) {
            logger_1.logger.error(`Error updating stock for part ${id}:`, error);
            throw new Error(`Failed to update stock for part ${id}`);
        }
    }
    async adjustStock(id, adjustment) {
        try {
            logger_1.logger.info(`Adjusting stock for part ${id} by ${adjustment}`);
            const response = await this.client.put(`/api/parts/${id}/adjust-stock`, { adjustment });
            return response.data;
        }
        catch (error) {
            logger_1.logger.error(`Error adjusting stock for part ${id}:`, error);
            throw new Error(`Failed to adjust stock for part ${id}`);
        }
    }
    async checkAvailability(id, requiredQuantity) {
        try {
            logger_1.logger.info(`Checking availability for part ${id}, required: ${requiredQuantity}`);
            const response = await this.client.get(`/api/parts/${id}/availability?requiredQuantity=${requiredQuantity}`);
            return response.data.available;
        }
        catch (error) {
            logger_1.logger.error(`Error checking availability for part ${id}:`, error);
    ...
    async getAllCategories() {
        try {
            logger_1.logger.info('Fetching all categories');
    ...
    async getAllBrands() {
        try {
            logger_1.logger.info('Fetching all brands');
    ...
    async getHealthStatus() {
        try {
            logger_1.logger.info('Checking part service health');
            const response = await this.client.get('/api/parts/health');
            return response.data;
        }
        catch (error) {
            logger_1.logger.error('Error checking part service health:', error);
    ...
    async getPartStats() {
        try {
            logger_1.logger.info('Fetching part statistics');
            const response = await this.client.get('/api/parts/stats');
            return response.data;
        }
        catch (error) {
            logger_1.logger.error('Error fetching part stats:', error);
            throw new Error('Failed to fetch part statistics');
        }
    }
    async listParts(page, size, category, status, search) {
        try {
            logger_1.logger.info('Listing parts with filters:', { page, size, category, status, search });
            ...
            logger_1.logger.error('Error listing parts:', error);
            throw new Error('Failed to list parts');
        }
    }
    async getPartByNumber(partNumber) {
        return this.getPartByPartNumber(partNumber);
    }
    async getPartsByManufacturer(manufacturer) {
        return this.getPartsByBrand(manufacturer);
    }
    async updatePartStock(id, quantity, operation) {
        try {
            logger_1.logger.info(`Updating part stock: ${id}, quantity: ${quantity}, operation: ${operation}`);
            let response;
            switch (operation) {
                case 'SET':
            ...
            logger_1.logger.error(`Error updating part stock for ${id}:`, error);
            throw new Error(`Failed to update stock for part ${id}`);
        }
    }
    async searchParts(query, page, size) {
        try {
            logger_1.logger.info(`Searching parts with query: ${query}`);
            const params = new URLSearchParams();
            ...
            logger_1.logger.error('Error searching parts:', error);
            throw new Error('Failed to search parts');
        }
    }
    async getPartsByPriceRange(minPrice, maxPrice) {
        try {
            logger_1.logger.info(`Fetching parts in price range: ${minPrice} - ${maxPrice}`);
            const response = await this.client.get(`/api/parts/price-range?minPrice=${minPrice}&maxPrice=${maxPrice}`);
            return response.data;
        }
        catch (error) {
            logger_1.logger.error('Error fetching parts by price range:', error);
            throw new Error('Failed to fetch parts by price range');
        }
    }
    async getPartsBySupplier(supplier) {
        try {
            logger_1.logger.info(`Fetching parts by supplier: ${supplier}`);
            const response = await this.client.get(`/api/parts/supplier/${encodeURIComponent(supplier)}`);
            return response.data;
        }
        catch (error) {
            logger_1.logger.error(`Error fetching parts by supplier ${supplier}:`, error);
            throw new Error(`Failed to fetch parts from supplier ${supplier}`);
        }
    }
    async bulkUpdatePartPrices(priceAdjustment, category, manufacturer) {
        try {
            logger_1.logger.info('Bulk updating part prices:', { priceAdjustment, category, manufacturer });
            const params = new URLSearchParams();
            ...
            logger_1.logger.error('Error bulk updating part prices:', error);
            throw new Error('Failed to bulk update part prices');
        }
    }
    async getPartInventoryValue(category, manufacturer) {
        try {
            logger_1.logger.info('Fetching part inventory value:', { category, manufacturer });
            const params = new URLSearchParams();
            if (category)
                params.append('category', category);
            if (manufacturer)
            ...
            logger_1.logger.error('Error fetching part inventory value:', error);
